...
import {
  Search, Puzzle, BookOpen, ListOrdered, Code2, Eye,
  Film, Brain, ArrowDown, FileText, LayoutGrid,
} from "lucide-react";
...
const STEPS: StepDef[] = [
  ...
  },
  {
    ...
    inputs: ["idea", "idioma", "audiencia", "duración objetivo", "plugins aprobados"],
  ...
  },
  {
    ...
    icon: ListOrdered,
    label: "Beat Writer",
    role: "Divide cada escena en 2-5 beats (texto + animación) — el contrato de sincronía voz↔video",
    inputs: ["outline.md", "idioma", "duración objetivo"],
    outputs: ["beats/scene_NN.beats.json (id, text, anim_hint, duration_s_est)"],
    model: "claude sonnet",
  ...
  },
  {
    ...
    icon: Code2,
    label: "Coder",
    role: "Escribe VoiceoverScene con un `with self.voiceover()` por beat (hasta 3 ciclos de fix)",
    inputs: ["outline.md", "beats/scene_NN.beats.json", "plugins aprobados"],
    outputs: ["scenes/scene_NN.py (VoiceoverScene)", "renders/scene_NN/preview.mp4 (con audio sync)"],
    model: "claude opus",
  ...
  },
  {
    ...
    icon: LayoutGrid,
    label: "Scene Review",
    role: "El usuario aprueba o pide revisión de cada escena de forma independiente",
    inputs: ["preview.mp4 por escena", "beats asociados"],
    outputs: ["estado 'approved' por escena — desbloquea el render final"],
    model: "— (acción humana)",
    skillFiles: [],
    isGate: true,
    color: "text-teal-400 border-teal-800",
  ...
  },
  {
    ...
    icon: Film,
    label: "Editor",
    role: "Render -qh + concat (audio ya embebido por manim-voiceover, sin mux)",
    inputs: ["scene_NN.py (×N, todas aprobadas)"],
